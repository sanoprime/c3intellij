...
data class FullyQualifiedName(
    ...
    companion object {
        ...
        fun from(psi: C3FuncDef, module: ModuleName? = psi.moduleName): FullyQualifiedName {
            return FullyQualifiedName(module, checkNotNull(psi.funcHeader).funcName.text)
        }

        fun from(psi: C3MacroDefinition, module: ModuleName? = psi.moduleName): FullyQualifiedName {
            return FullyQualifiedName(module, checkNotNull(psi.macroHeader).macroName.text)
        }
...
data class ShortType(
    ...
    companion object {
        ...
        fun deserialize(string: String): ShortType {
            return ShortType(string)
        }

        fun deserialize(strings: List<String>): List<ShortType> {
            return strings.map { deserialize(it) }
        }
    }
}
