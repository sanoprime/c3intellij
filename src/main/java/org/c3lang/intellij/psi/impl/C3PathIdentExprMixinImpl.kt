...
abstract class C3PathIdentExprMixinImpl(node: ASTNode) : C3PsiElementImpl(node), C3PathIdentExpr {
    ...
    override val nameIdent: String?
        get() = lastChild.takeIf { it.elementType == C3Types.IDENT }?.text

    private class ImportPathReference(element: C3PathIdentExpr) : C3ReferenceBase<C3PathIdentExpr>(element) {
        override fun multiResolve(): Collection<C3PsiElement> {
            return element.importProvider.getImportOf(element)
        }

        override fun getRangeInElement(): TextRange {
            val path = element.pathIdent.path ?: return TextRange.EMPTY_RANGE
