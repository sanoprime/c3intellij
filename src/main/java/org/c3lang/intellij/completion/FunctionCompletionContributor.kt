...
//        psiElement(TokenType.WHITE_SPACE) // TODO more logic needed
    ...
    override fun addCompletions(
        ...
        val pathIdentContext = PathIdentContext(parameters)
        val importProvider = pathIdentContext.importProvider ?: return

        val matcher = NameUtil.buildMatcher(
            "*${pathIdentContext.lookupString}*",
            NameUtil.MatchingCaseSensitivity.NONE
        )
        val insertHandler = FunctionInsertHandler(
            moduleSection = importProvider,
            range = pathIdentContext.range
        )

        StubIndex.getInstance().getAllKeys(
            NameIndex.KEY,
            pathIdentContext.project
        ).filter { matcher.matches(it) || it.isBlank() }.flatMap { key ->
            StubIndex.getElements(
                NameIndex.KEY,
                key,
                pathIdentContext.project,
                GlobalSearchScope.allScope(pathIdentContext.project),
                C3PsiElement::class.java,
            )
        }.filterIsInstance<C3CallablePsiElement>().forEach { element ->
            val sameFileBonus = if (element.sourceFileName == pathIdentContext.containingFileName) 1.0 else 0.0
            val sameModuleBonus = if (element.moduleName == importProvider.moduleName) 1.0 else 0.0
            val importBonus = if (importProvider.imports.contains(element.moduleName)) 1.0 else 0.0
